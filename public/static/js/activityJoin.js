	    	...
	    	if (json.code == 200){
	    		...
	    		if(json.data.list != ''){
	    			$.each(json.data.list, function(key, val) {
	    				...
	    				html += '<td>' + (key+1) + '</td>';
	    				html += '<td>';
	    				html += '<a target="_blank" title="'+val.activity_name+'" href="/index/activity/detail/id/'+ val.activity_id +'">'+ val.activity_name_short +'</a>';
	    				if(val.at_id == '8') {
                        ...
                        if(val.pay_status == '0' && val.order_status == '0'){
                            html += '<a class="layui-btn layui-btn-sm layui-btn-warm pay_a" target="_blank" href="/index/activity/confirm/order_id/'+val.ap_id+'">去支付</a>';
                            html += '<a style="" class="layui-btn layui-btn-sm layui-btn-warm" onclick="cancelOrder('+val.ap_id+')">取消订单</a>';

                        }else if(val.order_status == 1){
                            html += '<a  class="layui-btn layui-btn-sm layui-btn-warm" href="/index/user/activityOrder/order_id/'+val.ap_id+'">查看详情</a>';
                        }else{
                        ...
                        /* if(val.pay_status == 1 && val.pay_add_date > '2021-03-15 00:00:00'){
                            if(val.need_pay == 1 && val.activity_invoice && val.activity_invoice.status == 1){
                                ...
                                html += '<a style="margin-top:10px;" class="layui-btn layui-btn-sm layui-btn-warm" href="/activity/invoice/'+val.ap_id+'/1">开发票</a><br/>';
...
function refund_order(id){
    ...
    $.get("/index/user_api/refundOrder?id=" + id + "&type=1", function(result) {
        layer.open({
            type: 1,
            title: '确认退款',
            area: ['454px', '350px'],
