        ...
        <div class="container service-enter">
            ...
            <form method="post" action="{{ route('blog.store') }}">
                    ...
                    <div class="content-right">
                        <select class="service-select input-value" title="分类" name="category_id">
                            ...
                            @foreach (categorys() as $id => $name)
                                <option value="{{ $id }}"
                                @if (old('category_id') == $id) selected="selected" @endif
                                >{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="display-flex flex-warp content-wrap">
                    ...
                    <div class="content-right">
                        <input type="text" placeholder="请输入" maxlength="50" class="input-commom input-value" title="标题：" name="title" value="{{ old('title') }}">
                    </div>
                </div>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('#container').html(@json(old('content')));
    </script>
